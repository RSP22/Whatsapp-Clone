...
class ChatActivity : AppCompatActivity() {
    ...
    private fun updateHighFive(id: String, status: Boolean) {
        getMessages(friendId.toString()).child(id).updateChildren(mapOf("liked" to status))
    }
    ...
    private fun sendMessage(msg: String) {
        ...
        getMessages(friendId.toString()).child(id).setValue(msgMap)
    ...
    private fun updateLastMessage(message: Message, mCurrentUid: String) {
        ...
        getInbox(friendId.toString(), mCurrentUid).addListenerForSingleValueEvent(object :
            ValueEventListener {
            override fun onCancelled(p0: DatabaseError) {}
